...
const ThreeDBackground = () => {
  ...
  useEffect(() => { 
    ...
    const geometry = new THREE.BoxGeometry(0.7, 0.7, 0.7, 12, 12, 12)
    ...
    const shaderMaterial = new THREE.RawShaderMaterial({
      ...
      void main(){
        ...
        vec3 nPosition = fbm_vec3(position, 2.0, uTime*0.01);

        vec4 modelPosition = modelMatrix * vec4(position, 1.0);
    
        modelPosition.z += sin(modelPosition.x * uFrequency.x - (2.5*uTime)) * 0.08; //wave
        modelPosition.z += sin(modelPosition.y * uFrequency.y - (2.5*uTime)) * 0.08; //wave
    
        float elevation = sin(modelPosition.x * uFrequency.x - (2.5*uTime)) * 0.08;
        elevation +=  modelPosition.z += sin(modelPosition.y * uFrequency.y - (2.5*uTime)) * 0.08;
